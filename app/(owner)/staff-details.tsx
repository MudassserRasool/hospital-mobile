...
import { mockStaffProfile, mockAttendanceSummary } from '@/utils/mockData';
...
export default function StaffDetailsScreen() {
  ...
  return (
    <ThemedView style={styles.container}>
      <Stack.Screen options={{ headerShown: true, title: 'Staff Details', headerBackTitle: 'Back' }} />
      
      <ScrollView contentContainerStyle={styles.content}>
        <Card style={styles.profileCard}>
          <Image source={{ uri: staff.avatar }} style={styles.avatar} />
          <ThemedText style={styles.name}>{staff.name}</ThemedText>
          <ThemedText style={styles.role}>{staff.role} • {staff.department}</ThemedText>
          <Badge label={staff.isActive ? 'Active' : 'Inactive'} variant="success" />
        </Card>
        ...
        <Card style={styles.section}>
          ...
          <View style={styles.statsGrid}>
            <View style={styles.statItem}>
              <ThemedText style={styles.statValue}>{attendance.presentDays}</ThemedText>
              <ThemedText style={styles.statLabel}>Present</ThemedText>
            </View>
            <View style={styles.statItem}>
              <ThemedText style={styles.statValue}>{attendance.totalHours}h</ThemedText>
        ...
        <View style={styles.actions}>
          <Button title="Edit Details" variant="outline" style={{ flex: 1 }} onPress={() => Alert.alert('Edit', 'Edit functionality')} />
          <Button title="Deactivate" variant="danger" style={{ flex: 1 }} onPress={() => Alert.alert('Deactivate', 'Deactivate staff')} />
        </View>
      </ScrollView>
    </ThemedView>
  );
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: FontSizes['2xl'], fontWeight: FontWeights.bold, marginBottom: 4 },
  role: { fontSize: FontSizes.md, color: NeutralColors.gray600, marginBottom: Spacing.md },
  ...
  statItem: { flex: 1, alignItems: 'center', padding: Spacing.md, backgroundColor: NeutralColors.gray50, borderRadius: BorderRadius.md },
  statValue: { fontSize: FontSizes.xl, fontWeight: FontWeights.bold, color: BrandColors.primary },
